  ![JavaScript面试题.png](./image/js.png)
...
CommonJs require后执行整个模块代码 且加载同一个模块只会执行一次，执行完进行缓存
ES6 Module 像const一样指定 import {xx} from 'xxx' 无法给xx赋值 但可以改变xx的属性和方法
...
- 变量环境也是一个词法环境，其环境记录器持有变量声明语句在执行上下文中创建的绑定关系。 var
